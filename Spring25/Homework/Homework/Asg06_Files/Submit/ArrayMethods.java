...
public class ArrayMethods {
    ...
    /**
     * Checks if the array of Rectangle objects is sorted in ascending order.
     * Uses isGreater(), isLess(), and equals() methods of Rectangle.
     * Throws IllegalArgumentException if the array is empty.
     * @param nums the array of Rectangle objects to check
    ...
    public static boolean isSorted(Rectangle[] nums) {
        ...
        for (int i = 0; i < nums.length - 1; i++) {
            // Use isGreater as required by the assignment
    ...
    /**
     * Checks if all elements in the integer array are unique.
    ...
    /**
     * Doubles the length and width of all squares (length == width) in the array.
     * Modifies the array in place.
     * @param shapes the array of Rectangle objects to modify
     */
    public static void doubleSquares(Rectangle[] shapes) {
        for (int i = 0; i < shapes.length; i++) {
            if (shapes[i].getLength() == shapes[i].getWidth()) {
                shapes[i].setLength(shapes[i].getLength() * 2);
                shapes[i].setWidth(shapes[i].getWidth() * 2);
            }
        }
    }

    /**
     * Removes all elements from the ArrayList that are in the range [from, to] (inclusive).
    ...
    /**
     * Repeats each string in the ArrayList k times, in order.
     * Modifies the ArrayList in place. Throws IllegalArgumentException if k <= 0.
     * Does not use any additional ArrayList objects.
